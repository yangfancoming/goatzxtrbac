...
public class SysDictTagProcessor extends AbstractElementTagProcessor {
    ...
    protected void doProcess(ITemplateContext iTemplateContext, IProcessableElementTag iProcessableElementTag, IElementTagStructureHandler iElementTagStructureHandler) {
        ...
        // 注入字典
        DictDetailService dictDetailService = applicationContext.getBean(DictDetailService.class);
        // 提交表单时的 name
        String fieldName = iProcessableElementTag.getAttributeValue("dictId"); // dictId
        // 元素的 class 样式     form-control m-input
        String dictClass = iProcessableElementTag.getAttributeValue("class");
        // 根据类型查询出字典列表
        List<SysDictDetail> dictList = dictDetailService.getDictDetail(Integer.valueOf(fieldName));
        // 创建将替换自定义标签的 DOM 结构
